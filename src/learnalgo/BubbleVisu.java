 ...
 * @author Dr D.Brown
 */
public class BubbleVisu extends Application {
    ...
    private static final Border ELEMENT_BORDER = new Border(new BorderStroke(Color.GREEN, BorderStrokeStyle.SOLID, new CornerRadii(BORDER_RADII), new BorderWidths(BORDER_WIDTH)));
...
//        final double ANIMATION_PANE_WIDTH = SPACING + (SPACING + ELEMENT_WIDTH) * NUM_OF_ELEMENTS;
//        final double ANIMATION_PANE_HEIGHT = SPACING * 10 + ELEMENT_HEIGHT + CURSOR_HEIGHT * 3;
    ...
    private void bubbleSort() {
        ...
        while (swapped) {
            ...
            for (int i = 0; i < n; i++) {
                ...
                if (elements[i].value.compareTo(elements[i + 1].value) > 0) {
                    Element temp = elements[i];
                    elements[i] = elements[i + 1];
                    elements[i + 1] = temp;
                    transitions.getChildren().add(swap(i, i + 1));
                    swapped = true;
                }
            }
            n--;
